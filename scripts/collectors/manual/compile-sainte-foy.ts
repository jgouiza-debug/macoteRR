 ...
 * supabase/seed/sainte-foy/*.json and writes staging rows. Per
 * docs/02-scraping-collection-plan.md, bursaries and program listings are compiled by hand,
 * not scraped -- this script is the mechanism that gets that hand-compiled data through the
 * same staging -> review -> promote pipeline every other source uses, rather than a raw
 * seed.sql that would bypass review entirely.
 *
...
type BursariesFile = {
  cegep_short_code: string;
  // Matches supabase/seed/sainte-foy/README.md's documented shape for this file.
  bursaries: {
    name: string;
    source_org: string;
...
async function main() {
  ...
  // --- programs.json -> staging_cegep_programs ---
  if (programsFile.programs.length > 0) {
    ...
    if (!cegep) {
      skipped.push(
        `All of programs.json: cegep short_code "${programsFile.cegep_short_code}" not found in production. Seed cluster 1 (cegeps) first.`,
      );
    } else {
  ...
  // --- cutoffs.json -> staging_university_programs + children ---
  for (const program of cutoffsFile.university_programs) {
    ...
    if (!university) {
      skipped.push(
        `${program.name}: university short_code "${program.university_short_code}" not found in production. Seed cluster 1 (universities) first.`,
      );
      continue;
    }
    ...
    // Prerequisites and grade floors reference courses by code, resolved against production.
    for (const courseCode of program.prerequisites) {
      ...
      const { error } = await supabase.from("staging_university_program_prerequisites").insert({
      ...
      if (error) skipped.push(`${program.name} prerequisite ${courseCode}: ${error.message}`);
    }
    ...
    for (const floor of program.grade_floors) {
      ...
      const { error } = await supabase.from("staging_university_program_grade_floors").insert({
        university_program_id: stagingProgram.id,
        course_id: course.id,
      ...
      if (error) skipped.push(`${program.name} grade floor ${floor.course_code}: ${error.message}`);
    }

    for (const entry of program.cutoff_history) {
      const { error } = await supabase.from("staging_cutoff_history").insert({
        university_program_id: stagingProgram.id,
      ...
      if (error) skipped.push(`${program.name} cutoff_history ${entry.admission_year}: ${error.message}`);
    }
  }

  // --- bursaries.json -> staging_bursaries ---
  if (bursariesFile.bursaries.length > 0) {
    ...
    if (!cegep) {
      skipped.push(
        `All of bursaries.json: cegep short_code "${bursariesFile.cegep_short_code}" not found in production. Seed cluster 1 (cegeps) first.`,
      );
    } else {
