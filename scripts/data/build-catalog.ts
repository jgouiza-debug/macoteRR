/**
 * Normalises the two scraped catalogues in src/lib/data/raw into:
 *
 *   1. src/lib/data/catalog.generated.ts  — the typed, client-safe catalogue the app reads
 *   2. supabase/seed/catalog.sql          — an idempotent seed for cegeps / cegep_programs /
 *                                           universities / university_programs
 *
 * Both outputs are generated, committed, and must never be hand-edited: rerun
 * `npm run build:catalog` after touching the raw JSON or the maps below.
 *
...
// ------------------------------------------------------- emit catalog.generated.ts

const ts = `// GENERATED by scripts/data/build-catalog.ts — do not edit by hand.
...
export const CATALOG_CEGEPS: CatalogCegep[] = ${JSON.stringify(
  cegeps.map(({ programs: _programs, ...rest }) => rest),
  null,
  2,
)};

export const CATALOG_CEGEP_PROGRAMS: CatalogCegepProgram[] = ${JSON.stringify(
  cegeps.flatMap((c) => c.programs.map(({ summary: _summary, ...rest }) => rest)),
  null,
  2,
)};

export const CATALOG_UNIVERSITIES: CatalogUniversity[] = ${JSON.stringify(
  allUniversities.map(({ programs: _programs, ...rest }) => rest),
  null,
  2,
)};

export const CATALOG_UNIVERSITY_PROGRAMS: CatalogUniversityProgram[] = ${JSON.stringify(
  allUniversities.flatMap((u) => u.programs),
  null,
  2,
)};
`;

writeFileSync(join(ROOT, "src", "lib", "data", "catalog.generated.ts"), ts, "utf8");

// Program blurbs live in their own module so the onboarding pickers — which only ever render
// names — do not drag ~40KB of scraped prose into the client bundle.
const summaries = `// GENERATED by scripts/data/build-catalog.ts — do not edit by hand.
// Program blurbs, split out of catalog.generated.ts so client pickers never bundle them.
// Scraped from each cégep's own program pages; treat as descriptive copy, not as a figure.

export const CEGEP_PROGRAM_SUMMARIES: Record<string, string> = ${JSON.stringify(
  Object.fromEntries(
    cegeps.flatMap((c) => c.programs.filter((p) => p.summary).map((p) => [p.id, p.summary])),
  ),
  null,
  2,
)};
...
writeFileSync(join(ROOT, "src", "lib", "data", "catalog-summaries.generated.ts"), summaries, "utf8");
...
console.log("wrote src/lib/data/catalog.generated.ts + catalog-summaries.generated.ts");
